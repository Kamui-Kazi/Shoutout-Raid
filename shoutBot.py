...
class shoutBot(commands.Bot):
    def __init__(self, *, token_database: asqlite.Pool, auth_mode:bool = False) -> None:
        ...
        self.raids = asyncio.Queue()
        self.shout_task:asyncio.Task = None
    ...
    async def start_shout_timer(self):
        try:
            while True:
                ...
                raid_payload:twitchio.ChannelRaid = await self.raids.get()
...
class MyComponent(commands.Component):
    ...
    async def event_raid(self, payload: twitchio.ChannelRaid) -> None:
        ...
        await self.bot.raids.put(payload)  # Add raid to the queue
...
def main(auth_mode:bool = False) -> None:
    ...
    async def runner() -> None:
        async with asqlite.create_pool("tokens.db") as tdb, shoutBot(auth_mode=auth_mode, token_database=tdb) as bot:
